...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> &predictions) {
  ...
  while (state != possible_successor_state.end()){
    ...
    cost_for_state.push_back(calculate_cost(*this, predictions, trajectory_for_state));
    ++ state;
  }
  int best_state_index = std::distance(cost_for_state.begin() , std::min_element(cost_for_state.begin(), cost_for_state.end()));
  

  /**
   * TODO: Change return value here:
   */
  return generate_trajectory(possible_successor_state[best_state_index], predictions);
}
